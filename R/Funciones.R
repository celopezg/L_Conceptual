...
############### 1.1. Construir tabla 2 variables ###############

tabla_2 <- function(datos, categoria, cantidad, variable,  mensaje, titulo){
  tba0 <- datos %>% spread(categoria,cantidad)
  n <- datos %>% ungroup() %>% select(categoria) %>% distinct() %>% nrow()
  categorias <- (datos %>% ungroup() %>% select("CAT"=categoria) %>% distinct())$CAT
  sketch = htmltools::withTags(table(
    class = 'display',
    thead(
      tr(
        th(rowspan = 2, 'Año'),
        th(rowspan = 2, 'Periodo'),
        th(rowspan = 2, colnames(tba0)[3]),
        th(colspan = n, variable)
      ),
      tr(
        lapply(categorias, th)
      )
  ...
  tba1 <- tba0
  tba1[,1:3] <- data.frame(apply(tba1[,1:3],2,as.factor) )
  tba1 <- tba1 %>% select(colnames(tba0[,1:3]),categorias)
  tba <- datatable(
    tba1, 
    fillContainer = paste0("Tabla: ",mensaje),
    filter = 'top',
    ...
    container = sketch,
    rownames = FALSE,
    options = list(columnDefs = list(list(className = 'dt-center', targets = 0:(n+1))),
                   order = list(list(0, 'desc'),list(1, 'desc')),
                   searchHighlight = TRUE,
                   pageLength = 10,
                   keys = TRUE,
...
############### 1.2. Construir tabla n variables ###############
...
tabla_n <- function(datos, categorias, variable,  mensaje, titulo){
  ...
  tba1 <- datos
  tba2 <- data.frame(apply(tba1[,-6],2,as.factor),Total=tba1$Total)
  tba <- datatable(
    tba2, 
    fillContainer = paste0("Tabla: ",mensaje),
    filter = 'top',
...
torta <- function(datos,variable,colores,titulo,etiqueta,ano,periodo,periodo_titulo){
  ...
  gfa_final <- highchart()  %>%
    hc_title(style = list( fontWeight = "bold",
                           fontSize = "22px", 
                           useHTML = TRUE ), 
             text = paste0(titulo, '', periodo_titulo)) %>% 
...
barra_horizontal <- function(datos,categoria,colores,ano,periodo,periodo_titulo,titulo,eje){
  ...
  gfa <- highchart()  %>%
    ...
    hc_title(style = list( fontWeight = "bold",fontSize = "25px" ), text = paste0(titulo,'',periodo_titulo)) %>%
